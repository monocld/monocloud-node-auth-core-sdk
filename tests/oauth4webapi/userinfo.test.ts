...
test("userInfoRequest() w/ jwt signal", async (t) => {
  ...
  const tIssuer: lib.AuthorizationServer = {
    ...issuer,
    userinfo_endpoint: endpoint("userinfo-2"),
  };
  const tClient: lib.Client = {
    ...client,
...
test("processUserInfoResponse() - json", async (t) => {
  ...
  await t.throwsAsync(
    lib.processUserInfoResponse(tIssuer, client, "sub", <any>null),
    {
      message: '"response" must be an instance of Response',
    }
  );
  await t.throwsAsync(
    lib.processUserInfoResponse(tIssuer, client, "sub", getResponse('{"')),
    {
  ...
  await t.throwsAsync(
    lib.processUserInfoResponse(
      ...
      client,
      "sub",
      getResponse(JSON.stringify({ sub: "urn:example:subject" }), {
        status: 404,
      })
    ),
    {
  ...
  await t.notThrowsAsync(async () => {
    const response = getResponse(
      JSON.stringify({ sub: "urn:example:subject" })
    );
  ...
  await t.throwsAsync(
    async () => {
      const response = getResponse(
        JSON.stringify({ sub: "urn:example:different-subject" })
      );
      await lib.processUserInfoResponse(
        tIssuer,
        client,
