...
test("introspectionRequest() w/ Custom Headers", async (t) => {
  ...
  for (const headers of [
    ...
    await t.notThrowsAsync(
      lib.introspectionRequest(tIssuer, tClient, "token", { headers })
    );
  }
});
...
test("processIntrospectionResponse()", async (t) => {
  await t.throwsAsync(
    lib.processIntrospectionResponse(issuer, client, <any>null),
    {
  ...
  await t.throwsAsync(
    lib.processIntrospectionResponse(issuer, client, getResponse('{"')),
    {
      message: 'failed to parse "response" body as JSON',
    }
  );
  await t.throwsAsync(
    lib.processIntrospectionResponse(issuer, client, getResponse("null")),
    {
