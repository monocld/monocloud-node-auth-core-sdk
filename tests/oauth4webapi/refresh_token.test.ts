...
test("refreshTokenGrantRequest()", async (t) => {
  await t.throwsAsync(
    lib.refreshTokenGrantRequest(issuer, tClient, "refresh_token"),
    {
  ...
  await t.throwsAsync(
    lib.refreshTokenGrantRequest(issuer, tClient, <any>null),
    {
...
test("refreshTokenGrantRequest() w/ DPoP", async (t) => {
  ...
  await t.notThrowsAsync(
    lib.refreshTokenGrantRequest(tIssuer, tClient, "refresh_token", { DPoP })
  );
});

test("processRefreshTokenResponse() without ID Tokens", async (t) => {
  await t.throwsAsync(
    lib.processRefreshTokenResponse(issuer, client, <any>null),
    {
  ...
  await t.throwsAsync(
    lib.processRefreshTokenResponse(issuer, client, getResponse('{"')),
    {
      message: 'failed to parse "response" body as JSON',
    }
  );
  await t.throwsAsync(
    lib.processRefreshTokenResponse(issuer, client, getResponse("null")),
    {
