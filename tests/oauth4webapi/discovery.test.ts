...
test("processDiscoveryResponse()", async (t) => {
  ...
  await t.throwsAsync(
    lib.processDiscoveryResponse(expected, getResponse("", { status: 404 })),
    {
      message:
  ...
  await t.throwsAsync(
    lib.processDiscoveryResponse(expected, getResponse('{"')),
    {
      message: 'failed to parse "response" body as JSON',
    }
  );
  await t.throwsAsync(
    lib.processDiscoveryResponse(expected, getResponse("null")),
    {
  ...
  await t.throwsAsync(
    lib.processDiscoveryResponse(
      expected,
      getResponse(JSON.stringify({ issuer: null }))
    ),
    {
