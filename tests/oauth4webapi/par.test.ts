...
test("pushedAuthorizationRequest()", async (t) => {
  await t.throwsAsync(
    lib.pushedAuthorizationRequest(issuer, tClient, new URLSearchParams()),
    {
  ...
  await t.notThrowsAsync(
    lib.pushedAuthorizationRequest(tIssuer, tClient, [
      ["response_type", "code"],
    ])
  );
});
...
test("pushedAuthorizationRequest() w/ Request Object", async (t) => {
  ...
  const request = await lib.issueRequestObject(
    tIssuer,
    tClient,
    new URLSearchParams(),
    {
...
test("processPushedAuthorizationResponse()", async (t) => {
  await t.throwsAsync(
    lib.processPushedAuthorizationResponse(issuer, client, <any>null),
    {
  ...
  await t.throwsAsync(
    lib.processPushedAuthorizationResponse(
      issuer,
      client,
      getResponse(JSON.stringify({ request_uri: null, expires_in: 60 }), {
        status: 201,
      })
    ),
    {
  ...
  await t.throwsAsync(
    lib.processPushedAuthorizationResponse(
      ...
      client,
      getResponse(
        JSON.stringify({ request_uri: "urn:example:uri", expires_in: null }),
        {
          status: 201,
        }
      )
  ...
  t.deepEqual(
    await lib.processPushedAuthorizationResponse(
      ...
      client,
      getResponse(
        JSON.stringify({ request_uri: "urn:example:uri", expires_in: 60 }),
        {
          status: 201,
        }
      )
  ...
  t.true(
    lib.isOAuth2Error(
      await lib.processPushedAuthorizationResponse(
        issuer,
        client,
        getResponse(JSON.stringify({ error: "invalid_client" }), {
          status: 401,
        })
      )
    )
