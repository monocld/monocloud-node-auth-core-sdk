...
test("validateJwtAuthResponse() error conditions", async (t) => {
  await t.throwsAsync(
    () => lib.validateJwtAuthResponse(issuer, client, <any>null),
    {
      message: '"parameters" must be an instance of URLSearchParams, or URL',
    }
  );
  await t.throwsAsync(
    () => lib.validateJwtAuthResponse(issuer, client, new URLSearchParams()),
    {
  ...
  await t.throwsAsync(
    () =>
      lib.validateJwtAuthResponse(
        issuer,
        client,
        new URLSearchParams("response=foo")
      ),
    {
...
test("validateJwtAuthResponse()", async (t) => {
  ...
  await t.notThrowsAsync(async () => {
    const result = await lib.validateJwtAuthResponse(
      tIssuer,
      client,
      params,
      lib.expectNoState
...
test("validateJwtAuthResponse() - state value", async (t) => {
  ...
  await t.notThrowsAsync(
    lib.validateJwtAuthResponse(tIssuer, client, params, "state")
  );
});
...
test("validateJwtAuthResponse() - state ignored", async (t) => {
  ...
  const params = new URLSearchParams({ response });
  await t.notThrowsAsync(
    lib.validateJwtAuthResponse(tIssuer, client, params, lib.skipStateCheck)
  );
});
