...
test("parseWwwAuthenticateChallenges()", (t) => {
  {
    ...
    t.deepEqual(
      lib.parseWwwAuthenticateChallenges(response(headers)),
      undefined
    );
  }
...

  {
    ...
    t.deepEqual(lib.parseWwwAuthenticateChallenges(response(headers)), [
      {
        scheme: "basic",
        parameters: { charset: "UTF-8", realm: "Access to the staging site" },
      },
    ]);
  }
...

  {
    ...
    t.deepEqual(lib.parseWwwAuthenticateChallenges(response(headers)), [
      {
        scheme: "newauth",
        parameters: { realm: "apps", type: "1", title: 'Login to "apps",' },
      },
      {
        scheme: "newauth",
        parameters: { realm: "apps", type: "1", title: 'Login to "apps' },
...

  {
    ...
    headers.append(
      name,
      'Newauth realm="apps", type=1, title="Login to apps=asd"'
    );
    t.deepEqual(lib.parseWwwAuthenticateChallenges(response(headers)), [
      {
        scheme: "newauth",
        parameters: { realm: "apps", type: "1", title: "Login to apps=asd" },
      },
    ]);
  }

  {
    ...
    headers.append(
      name,
      'Newauth realm="apps", title="Login to, apps=asd", type=1'
    );
    t.deepEqual(lib.parseWwwAuthenticateChallenges(response(headers)), [
      {
        scheme: "newauth",
