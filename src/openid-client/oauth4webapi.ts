...
const dpopNonces = new LRU<string, string>(100);
...
function prepareHeaders(
  input?: [string, string][] | Record<string, string> | Headers
): Headers {
  if (looseInstanceOf(input, Headers)) {
    input = Object.fromEntries((input as any).entries());
  }
...
function signal(
  value: Exclude<HttpRequestOptions['signal'], undefined>
): AbortSignal {
...
export async function discoveryRequest(
  ...
  return (options?.[customFetch] || fetch)(url.href, {
    ...
    method: 'GET',
    redirect: 'manual',
    signal: (options?.signal ? signal(options.signal) : null) as any,
...
function getKeyAndKid(
  input: CryptoKey | PrivateKey | undefined
): NormalizedKeyInput {
...
export async function issueRequestObject(
  ...
  {
    const value = parameters.get('max_age');
    if (value !== null) {
  ...
  {
    const value = parameters.get('authorization_details');
    if (value !== null) {
      try {
...
export async function refreshTokenGrantRequest(
  ...
  const parameters = new URLSearchParams(options?.additionalParameters as any);
...
export function getValidatedIdTokenClaims(
  ref: OpenIDTokenEndpointResponse
): IDToken;
...
export async function clientCredentialsGrantRequest(
  ...
  return tokenEndpointRequest(
    ...
    client,
    'client_credentials',
    new URLSearchParams(parameters as any),
    options
  );
}
...
export async function revocationRequest(
  ...
  const body = new URLSearchParams(options?.additionalParameters as any);
...
async function validateJwt(
  ...
  getKey:
    | ((h: CompactJWSHeaderParameters) => Promise<CryptoKey>)
    | typeof noSignatureCheck,
  clockSkew: number,
  clockTolerance: number
 ...
 */
function getURLSearchParameter(
  parameters: URLSearchParams,
  name: string
): string | undefined {
...
export async function generateKeyPair(
  ...
  const algorithm:
    ...
    | EcKeyGenParams
    | AlgorithmIdentifier = algToSubtle(
    alg,
